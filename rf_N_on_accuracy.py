...
if __name__ == '__main__':
    ...
    for i in range(1,8):
        ...
        if i==1:
            image=newimage[0]
            label=np.zeros(len(newimage[0]),'i8')
            label=[x+1 for x in label]
        else:
            newlabel=np.zeros(len(newimage[0]),'i8')
    ...
    sc=np.zeros(10)
    for j in range(10):
        rf=RandomForestClassifier(n_estimators=n,oob_score=True,max_features=30,min_impurity_decrease=0.000001)
        ...
        for i in range(len(y_test)):
            if(y_test[i]==y_predict[i]):
                hit=hit+1
        print('ACCURACY OF RF:',hit/len(y_test))
        sc[j]=hit/len(y_test)
        n=n*2
    plt.plot(range(1,11),sc)
    plt.xlabel('log(N)/log(2)')
    plt.ylabel('accuracy')
    plt.show()
